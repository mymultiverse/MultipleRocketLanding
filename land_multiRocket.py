...
class ContactDetector(contactListener):
	...
	def BeginContact(self, contact):
		if self.env.water in [contact.fixtureA.body, contact.fixtureB.body] \
				...
				or self.env.lander1 in [contact.fixtureA.body, contact.fixtureB.body] \
			...
			for i in range(2):
				...
				if self.env.legs1[i] in [contact.fixtureA.body, contact.fixtureB.body]:
...
class TwoRocketLander(gym.Env):
	...
	def _destroy(self):
		...
		self.lander1 = None
		self.world.DestroyBody(self.legs1[0])
		self.world.DestroyBody(self.legs1[1])
		self.legs1 = []
	...
	def reset(self):
		...
		initial_x1 = W / 2 + W * np.random.uniform(-0.3, 0.3)


		self.lander1 = self.world.CreateDynamicBody(
			position=(initial_x1, initial_y),
			angle=0.0,
			fixtures=fixtureDef(
				shape=polygonShape(vertices=((-ROCKET_WIDTH / 2, 0),
											 (+ROCKET_WIDTH / 2, 0),
		...
		self.lander1.color1 = rgb(230, 230, 230)
		...
		for i in [-1, +1]:
			...
			leg1 = self.world.CreateDynamicBody(
				position=(initial_x1 - i * LEG_AWAY, initial_y + ROCKET_WIDTH * 0.2),
				angle=(i * BASE_ANGLE),
				fixtures=fixtureDef(
					shape=polygonShape(
						vertices=((0, 0), (0, LEG_LENGTH / 25), (i * LEG_LENGTH, 0), (i * LEG_LENGTH, -LEG_LENGTH / 20),
			...
			djd1 = distanceJointDef(bodyA=self.lander1,
								   bodyB=leg1,
								   anchorA=(i * LEG_AWAY, ROCKET_HEIGHT / 8),
								   anchorB=leg1.fixtures[0].body.transform * (i * LEG_LENGTH, 0),
								   collideConnected=False,
								   frequencyHz=0.01,
								   dampingRatio=0.9
								   )
			...
			if i == 1:
				...
				rjd.upperAngle = 0
				rjd1.lowerAngle = -SPRING_ANGLE
				rjd1.upperAngle = 0
			else:
				rjd.lowerAngle = 0
				rjd.upperAngle = + SPRING_ANGLE
				rjd1.lowerAngle = 0
				rjd1.upperAngle = + SPRING_ANGLE

			leg.joint = self.world.CreateJoint(rjd)
			leg.joint2 = self.world.CreateJoint(djd)
   
			leg1.joint = self.world.CreateJoint(rjd1)
			leg1.joint2 = self.world.CreateJoint(djd1)

			self.legs.append(leg)
			self.legs1.append(leg1)
		...
		self.lander1.linearVelocity = (
			-self.np_random.uniform(0, INITIAL_RANDOM) * START_SPEED * (initial_x1 - W / 2) / W,
			-START_SPEED)
	...
	def step(self, action):
		...
		force_pos1 = (self.lander1.position[0], self.lander1.position[1])
		force1 = (-np.sin(self.lander1.angle + self.gimbal) * MAIN_ENGINE_POWER * self.power,
				 np.cos(self.lander1.angle + self.gimbal) * MAIN_ENGINE_POWER * self.power)
		self.lander1.ApplyForce(force=force1, point=force_pos1, wake=False)
		...
		force_pos_c1 = self.lander1.position + THRUSTER_HEIGHT * np.array(
			(np.sin(self.lander1.angle), np.cos(self.lander1.angle)))
		force_c1 = (-self.force_dir * np.cos(self.lander1.angle) * SIDE_ENGINE_POWER,
				   self.force_dir * np.sin(self.lander1.angle) * SIDE_ENGINE_POWER)
		self.lander1.ApplyLinearImpulse(impulse=force_c1, point=force_pos_c1, wake=False)
		...
		pos1 = self.lander1.position
		vel_l1 = np.array(self.lander1.linearVelocity) / START_SPEED
		vel_a1 = self.lander1.angularVelocity
		x_distance1 = (pos1.x - W / 2) / W
		y_distance1 = (pos1.y - self.shipheight) / (H - self.shipheight)

		angle = (self.lander.angle / np.pi) % 2
		if angle > 1:
			angle -= 2

		angle1 = (self.lander1.angle / np.pi) % 2
		if angle1 > 1:
			angle1 -= 2
	...
	def render(self, mode='human', close=False):
		...
		if self.viewer is None:
			...
			self.rockettrans1 = rendering.Transform()
			engine1 = rendering.FilledPolygon(((0, 0),
											  (ENGINE_WIDTH / 2, -ENGINE_HEIGHT),
											  (-ENGINE_WIDTH / 2, -ENGINE_HEIGHT)))
			self.enginetrans1 = rendering.Transform()
			engine1.add_attr(self.enginetrans1)
			engine1.add_attr(self.rockettrans1)
			engine1.set_color(.4, .4, .4)
			self.viewer.add_geom(engine1)

		   
			self.fire1 = rendering.FilledPolygon(((ENGINE_WIDTH * 0.4, 0), (-ENGINE_WIDTH * 0.4, 0),
												 (-ENGINE_WIDTH * 1.2, -ENGINE_HEIGHT * 5),
												 (0, -ENGINE_HEIGHT * 8), (ENGINE_WIDTH * 1.2, -ENGINE_HEIGHT * 5)))
			self.fire1.set_color(*rgb(255, 230, 107))
			self.firescale1 = rendering.Transform(scale=(1, 1))
			...
			smoke1 = rendering.FilledPolygon(((ROCKET_WIDTH / 2, THRUSTER_HEIGHT * 1),
											 (ROCKET_WIDTH * 3, THRUSTER_HEIGHT * 1.03),
											 (ROCKET_WIDTH * 4, THRUSTER_HEIGHT * 1),
											 (ROCKET_WIDTH * 3, THRUSTER_HEIGHT * 0.97)))
			smoke1.set_color(*self.sky_color_half_transparent)
			self.smokescale1 = rendering.Transform(scale=(1, 1))
			smoke1.add_attr(self.smokescale1)
			smoke1.add_attr(self.rockettrans1)

			self.viewer.add_geom(smoke)
			self.viewer.add_geom(smoke1)
			...
			for i in (-1, 1):
				...
				gridfin1 = rendering.FilledPolygon(finpoly)
				gridfin1.add_attr(self.rockettrans1)
				gridfin1.set_color(0.25, 0.25, 0.25)

				self.gridfins.append(gridfin)
				self.gridfins1.append(gridfin1)
			...
			# 		(i * ROCKET_WIDTH * 1.1, THRUSTER_HEIGHT * 0.99)
			# 	)
			# 	gridfin1 = rendering.FilledPolygon(finpoly1)
			# 	gridfin1.add_attr(self.rockettrans1)
			# 	gridfin1.set_color(0.25, 0.25, 0.25)
			# 	self.gridfins1.append(gridfin1)
		...
		if self.stepnumber % round(FPS / 10) == 0 and self.power > 0:
			...
			s1 = [MAX_SMOKE_LIFETIME * self.power,  # total lifetime
				 0,  # current lifetime
				 self.power * (1 + 0.2 * np.random.random()),  # size
				 np.array(self.lander1.position)
				 + self.power * ROCKET_WIDTH * 10 * np.array((np.sin(self.lander1.angle + self.gimbal),
															  -np.cos(self.lander1.angle + self.gimbal)))
				 + self.power * 5 * (np.random.random(2) - 0.5)]  # position
			self.smoke1.append(s1)

		for s,s1 in zip(self.smoke,self.smoke1):
			s[1] += 1
			s1[1] += 1
			if s[1] > s[0]:
				self.smoke.remove(s)
			...
			if s1[1] > s1[0]:
				self.smoke1.remove(s1)
				continue
			t1 = rendering.Transform(translation=(s1[3][0], s1[3][1] + H * s1[1] / 2000))
			self.viewer.draw_circle(radius=0.05 * s1[1] + s1[2],
									color=self.sky_color + (1 - (2 * s1[1] / s1[0] - 1) ** 2) / 3 * (
											self.sky_color_half_transparent - self.sky_color)).add_attr(t1)


		self.viewer.add_onetime(self.fire)
		self.viewer.add_onetime(self.fire1)

		for g,g1 in zip(self.gridfins,self.gridfins1):
		...
		for l1 in zip(self.legs1, [-1, 1]):
			path1 = [self.lander1.fixtures[0].body.transform * (l1[1] * ROCKET_WIDTH / 2, ROCKET_HEIGHT / 8),
					l1[0].fixtures[0].body.transform * (l1[1] * LEG_LENGTH * 0.8, 0)]
			self.viewer.draw_polyline(path1, color=self.ship.color1, linewidth=1 if START_HEIGHT > 500 else 2)
		...
		self.rockettrans1.set_translation(*self.lander1.position)
		self.rockettrans1.set_rotation(self.lander1.angle)
		self.enginetrans1.set_rotation(self.gimbal)
		self.firescale1.set_scale(newx=1, newy=self.power * np.random.uniform(1, 1.3))
		self.smokescale1.set_scale(newx=self.force_dir, newy=1)
